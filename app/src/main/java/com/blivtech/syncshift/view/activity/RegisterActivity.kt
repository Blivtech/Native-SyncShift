    ...
    class RegisterActivity : AppCompatActivity() {
        ...
        override fun onCreate(savedInstanceState: Bundle?) {
            ...
            binding.btnRegister.setOnClickListener {
                if (validateFields()) {
                    viewModel.postData(
                        "api/userdetails/save-registration/",
                        postObject()
                    )
                }
        ...
        private fun postObject(): JsonObject {
            ...
            return JsonObject().apply {
                ...
                addProperty("mobile_number", phone)
                addProperty("address", address)

                if (referral.isNotEmpty()) {
                    addProperty("referal_id", referral)
                }
        ...
        private fun validateFields(): Boolean {
            ...
            return when {
                name.isEmpty() -> {
                    binding.etName.error = "Enter Name"; false
                }
                phone.isEmpty() -> {
                    binding.etMobile.error = "Enter Phone Number"; false
                }
                username.isEmpty() -> {
                    binding.etUsername.error = "Enter Username"; false
                }
                password.isEmpty() -> {
                    binding.etPassword.error = "Enter Password"; false
                }
                address.isEmpty() -> {
                    binding.etAddress.error = "Enter Address"; false
                }
                else -> true
            }
        }
